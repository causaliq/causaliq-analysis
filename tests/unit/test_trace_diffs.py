...
# trace doesn't start with init
def test_trace_diffs_value_error_5():  # trace doesn't start with init
    trace = (
        Trace()
        .add(GraphAction.STOP, {GraphActionDetail.DELTA: 0.0})
        .add(GraphAction.STOP, {GraphActionDetail.DELTA: 10.0})
    )
    ref = (
        Trace()
...
def test_trace_diffs_value_error_6():  # trace doesn't start with init
    trace = (
        ...
        .add(
            GraphAction.ADD,
            {GraphActionDetail.ARC: ("A", "B"), GraphActionDetail.DELTA: 0.0},
        )
        .add(GraphAction.STOP, {GraphActionDetail.DELTA: 10.0})
    )
    ref = (
...
def test_trace_diffs_value_error_10():  # trace doesn't end with stop
    trace = (
        ...
        .add(
            GraphAction.ADD,
            {GraphActionDetail.ARC: ("A", "C"), GraphActionDetail.DELTA: 1.0},
        )
    )
    ref = (
        Trace()
...
# three entries identical
def test_trace_diffs_same_ok_2():  # three entries identical
    trace = (
        Trace()
        .add(GraphAction.INIT, {GraphActionDetail.DELTA: -10.12345678})
        .add(
            GraphAction.ADD,
            {GraphActionDetail.ARC: ("A", "B"), GraphActionDetail.DELTA: 1.0},
        )
...
def test_trace_diffs_same_ok_3():  # three identical entries
    trace = (
        ...
        .add(
            GraphAction.ADD,
            {GraphActionDetail.ARC: ("B", "C"), GraphActionDetail.DELTA: 1.0},
        )
        .add(GraphAction.STOP, {GraphActionDetail.DELTA: -9.12345678})
    )
    ref = (
...
def test_trace_diffs_same_ok_4():  # seven identical entries
    trace = (
        ...
        .add(
            GraphAction.ADD,
            {GraphActionDetail.ARC: ("A", "B"), GraphActionDetail.DELTA: 3.0},
        )
        .add(
            GraphAction.ADD,
            {GraphActionDetail.ARC: ("C", "D"), GraphActionDetail.DELTA: 2.0},
        )
        .add(
            GraphAction.ADD,
            {GraphActionDetail.ARC: ("B", "C"), GraphActionDetail.DELTA: 1.0},
        ...
        .add(
            GraphAction.REV,
            {GraphActionDetail.ARC: ("A", "B"), GraphActionDetail.DELTA: 0.5},
        )
        .add(
            GraphAction.DEL,
            {GraphActionDetail.ARC: ("C", "D"), GraphActionDetail.DELTA: 0.1},
        )
...
def test_trace_diffs_same_ok_5():  # small score differences
    ...
    ref = (
        Trace()
        .add(GraphAction.INIT, {GraphActionDetail.DELTA: 0.00003})
        .add(GraphAction.STOP, {GraphActionDetail.DELTA: 10.0})
    )
...
def test_trace_diffs_same_ok_6():  # small score differences
    trace = (
        Trace()
        .add(GraphAction.INIT, {GraphActionDetail.DELTA: -20002.1})
        .add(GraphAction.STOP, {GraphActionDetail.DELTA: -1800.3})
    )
    ref = (
        Trace()
        .add(GraphAction.INIT, {GraphActionDetail.DELTA: -20002.2})
...
def test_trace_diffs_same_ok_8():  # small score differences
    ...
    ref = (
        ...
        .add(
            GraphAction.DEL,
            {GraphActionDetail.ARC: ("C", "D"), GraphActionDetail.DELTA: 0.2},
        )
...
# ignore unmatched columns in trace
...
# ignore unmatched columns in ref
...
def test_trace_diffs_same_ok_11():  # accept None values
    trace = (
        ...
        .add(
            GraphAction.INIT,
            {
                GraphActionDetail.DELTA: -10.12345678,
    ...
    ref = (
        ...
        .add(
            GraphAction.STOP,
            {
                GraphActionDetail.DELTA: -9.12345678,
                GraphActionDetail.ARC: None,
            },
        )
    )
...
# trace has additional entry, same final
...
# trace has additional entry, diff final
...
# trace has missing entry, diff final
...
def test_trace_diffs_major_ok_4():  # different arcs added
    trace = (
        ...
        .add(
            GraphAction.ADD,
            {GraphActionDetail.ARC: ("A", "B"), GraphActionDetail.DELTA: 2.0},
        )
...
def test_trace_diffs_blocked_ok_4():  # one None, one [] - diffs
    ...
    trace = (
        ...
        .add(
            GraphAction.STOP,
            {
                GraphActionDetail.DELTA: -8.12345678,
                GraphActionDetail.BLOCKED: [],
            },
        )
    )
...
# significant difference in block score
...
# insignif. difference in block score
