...
# Test Bayesys metrics for single edge comparisons with different types
...
# Test Bayesys metrics for DHS dataset comparison
def test_core_metrics_bayesys_dhs():
    d8atr_fges = read(TESTDATA_DIR + "d8atr-fges.csv")
    d8atr_fges3 = read(TESTDATA_DIR + "d8atr-fges3.csv")
    assert values_same(
        pdag_compare(d8atr_fges, d8atr_fges3, bayesys="v1.5+")["bsf"], 0.541
    )
    assert values_same(
        pdag_compare(d8atr_fges3, d8atr_fges, bayesys="v1.5+")["bsf"], 0.252
    )


# Test Bayesys metrics for Sachs dataset with DAG and PDAG comparisons
def test_core_metrics_bayesys_sachs():
    dag = read_bn(TESTDATA_DIR + "sachs.dsc").dag
    print("\n\n{}".format(dag))
