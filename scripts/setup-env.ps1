# Comprehensive Python environment management for causaliq-analysis
# Usage: .\scripts\setup-env.ps1 [options]

param(
    [switch]$Install,           # Install causaliq-analysis package after creating environments
...
function Show-Help {
    ...
    Write-Host "  -Install         Create environments AND install causaliq-analysis package" -ForegroundColor White
    Write-Host "  -InstallOnly     Install causaliq-analysis package in existing environments" -ForegroundColor White
    Write-Host "  -Help            Show this help message" -ForegroundColor White
    Write-Host ""
    Write-Host "EXAMPLES:" -ForegroundColor Yellow
    Write-Host "  .\scripts\setup-env.ps1                    # Create environments only" -ForegroundColor Gray
    Write-Host "  .\scripts\setup-env.ps1 -Install           # Create + install causaliq-analysis" -ForegroundColor Gray
    Write-Host "  .\scripts\setup-env.ps1 -InstallOnly       # Install causaliq-analysis in existing envs" -ForegroundColor Gray
    Write-Host ""
...
function Install-InEnvironment {
    ...
    try {
        ...
        # Install the package with dependencies (using Test PyPI for dev releases)
        Write-Host "  Installing causaliq-analysis with dev dependencies..." -ForegroundColor Gray
...
if ($InstallOnly) {
    ...
    Write-Host "Installing causaliq-analysis in all environments..." -ForegroundColor Blue
    ...
    if ($Install) {
        Write-Host "(Will also install causaliq-analysis package)" -ForegroundColor Gray
    }
...
if (-not ($Install -or $InstallOnly)) {
    Write-Host "To install causaliq-analysis in all environments:" -ForegroundColor White
